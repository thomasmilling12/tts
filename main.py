...
@bot.tree.command(name="set_theme", description="Set the current meet theme")
async def set_theme(interaction: discord.Interaction, theme: str):
    settings = get_guild_settings(interaction.guild.id)
    settings["meet_theme"] = theme
    save_settings()
    await interaction.response.send_message(f"Theme set to: {theme}")

@bot.tree.command(name="set_host", description="Set the meet host")
async def set_host(interaction: discord.Interaction, host: str):
    settings = get_guild_settings(interaction.guild.id)
    settings["meet_host"] = host.lower()
    save_settings()
    await interaction.response.send_message(f"Host set to: {host}")

@bot.tree.command(name="start_meet", description="Start the meet")
async def start_meet(interaction: discord.Interaction):
    settings = get_guild_settings(interaction.guild.id)

    theme = settings.get("meet_theme", "Open Class")
    host = settings.get("meet_host", "Unknown")

    msg = f"The meet is now starting. Theme is {theme}. Hosted by {host}."

    await play_tts_text(interaction.guild, msg)
    await interaction.response.send_message("Meet started.")

@bot.tree.command(name="roll_call", description="Start roll call")
async def roll_call(interaction: discord.Interaction):
    msg = "Roll call starting. Stay in voice and get ready."
    await play_tts_text(interaction.guild, msg)
    await interaction.response.send_message("Roll call sent.")

@bot.tree.command(name="pick_winner", description="Announce a winner")
async def pick_winner(interaction: discord.Interaction, name: str, category: str = "Best Build"):
    settings = get_guild_settings(interaction.guild.id)

    settings["last_winner"] = f"{name} - {category}"
    save_settings()

    msg = f"Winner selected. {name} wins {category}."

    await play_tts_text(interaction.guild, msg)
    await interaction.response.send_message("Winner announced.")
